...
class Ajax extends MY_Controller
{
    ...
    public function examsMultiple(){
        ...
        if (!empty($branchID)) {
            ...
            $result = $this->db->get('exam')->result_array();
            if (count($result)) {
                $html .= '<option value="" default disabled>' . translate('select') . '</option>';
                foreach ($result as $row) {
                    if ($row['term_id'] != 0) {
                        $term = $this->db->select('name')->where('id', $row['term_id'])->get('exam_term')->row()->name;
                        $name = $row['name'] . ' (' . $term . ')';
    ...
    public function getDataByBranch()
    {
        ...
        if (!empty($branch_id)) {
            ...
            if (count($result)) {
                $html .= "<option value=''>" . translate('select') . "</option>";
                foreach ($result as $row) {
                $sel = ($row['id']  == $subject_id ? 'selected' : '');
    ...
    public function getSectionByClass()
    {
        ...
        if (!empty($classID)) {
            ...
            if (count($result)) {
                ...
                foreach ($result as $row) {
                	$sel = ($row['section_id']  == section_id ? 'selected' : '');
                    $html .= '<option value="' . $row['section_id'] . '" '.$sel.'>' . $row['name'] . '</option>';
                }
            } else {
	...
	public function getStudentByClassForExtraSubject()
    {
        $html = "";
        $class_id = $this->input->post('class_id');
        $section_id = $this->input->post('section_id');
