...
class Fees extends Admin_Controller
{
	...
	function add_library_fees(){
  
       
        $this->form_validation->set_rules('library_fees_amount', 'Library Fees Amount', array('trim','required','numeric','greater_than[0]'));
       $this->form_validation->set_rules('fees_type', 'Fees Name', array('trim','required'));
        if ($this->form_validation->run() !== false) {
            
            $amount = $this->input->post('library_fees_amount');
        	...
        	/*$addarrayFees = array(
                'name' => $fees_type,
                'fee_code' => $fees_type,
                'description' => $fees_type,
                'branch_id' => $branch_id,
                'created_at' => date('Y-m-d h:i:s')
            );
            $this->db->insert('fees_type', $addarrayFees);*/
            $fee_id = $fees_type;
            ...
            $this->db->insert('fee_groups_details', $arrayFees);
        	 $fee_groups_details_id = $this->db->insert_id();
			...
			$arrayFees = array(
                ...
                'remarks' => '',
            	'student_id'=>$student_id,	
            	'fee_groups_details_id' => $fee_groups_details_id,
    ...
    public function payment_history()
    {
        ...
        if ($this->input->post('search')) {
            ...
            $this->data['invoicelist'] = $this->fees_model->getStuPaymentHistory($classID, "", $paymentVia, $start, $end, $branchID,false, $fees_type,$staff_id);
        }
        $this->data['branch_id'] = $branchID;
	...
	public function delete_extra_fees($id){
    	
        $this->db->where('id', $id);
        $this->db->delete('fee_groups_details');
     	
    	$this->db->where('fee_groups_details_id', $id);
    	$this->db->delete('fee_payment_history');
    }
