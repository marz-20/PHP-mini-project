...
class Exam extends Admin_Controller
{
    ...
    public function CBSC_marksheet()
    {
        if (!get_permission('CBSC_report_card', 'is_view')) {
            access_denied();
        }
        $branchID = $this->application_model->get_branch_id();
        if ($_POST) {
        ...
        $this->data['branch_id'] = $branchID;
        $this->data['sub_page'] = 'exam/CBSC_reportCard';
        $this->data['main_menu'] = 'exam_reports';
        $this->data['title'] = translate('CBSC_report_card');
        $this->load->view('layout/index', $this->data);
    }
