...
<?php endif; ?>
        <li>
				<a href="#library_fees" data-toggle="tab"><i class="fas fa-hand-holding-usd"></i> Add Extra Fees</a>
			</li>
		</ul>
								...
								<tbody>
									<?php
                                       ...
                                       $allocation_id = $allocations[0]['allocation_id'];
                                        ...
                                        $group_id = $allocations[0]['group_id'];
										foreach ($allocations as $row) {
                                        if($row['student_id']>0){
                                            
                                        	if($row['student_id'] != $basic['id']){
                                            	continue;
                                            }
                                        }
									...
									<tr>
										...
										<td class="text-weight-semibold text-dark"><?=$row['name']?>
                                    		<?php
                                        	if($row['student_id']>0){
                                            	 echo btn_delete('fees/delete_extra_fees/'. $row['fees_g_id']);
                                            }
                                        	?>
                                   		 </td>
        ...
        <div id="library_fees" class="tab-pane">
					<?php echo form_open('fees/add_library_fees', array('class' => 'form-horizontal frm-submit' )); ?>
        				<input type="hidden" name="student_id" value="<?=$basic['id']?>">
        				<input type="hidden" name="branch_id" value="<?=$basic['branch_id']?>">
        				<input type="hidden" name="allocation_id" value="<?=$allocation_id?>">
                		<input type="hidden" name="fee_groups_id" value="<?=$group_id?>">
						<div class="form-group">
							<label class="col-md-3 control-label">Fees Type <span class="required">*</span></label>
							<div class="col-md-6">
								<?php
 									$sql = "SELECT * from fees_type where branch_id = ".$basic['branch_id'];
								...
								echo form_dropdown("fees_type", $feesArray, set_value('fees_type'), "class='form-control' id='extra_feesType'
								data-plugin-selectTwo data-width='100%' ");
							?>
								<span class="error"></span>
							</div>
					   ...
					   <div class="form-group">
							...
							<div class="col-md-6">
								<input type="text" class="form-control" name="library_fees_amount" id="library_fees_amount" value="0" autocomplete="off" />
								<span class="error"></span>
							</div>
						</div>
						<footer class="panel-footer">
