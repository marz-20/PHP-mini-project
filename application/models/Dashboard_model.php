...
class Dashboard_model extends CI_Model
{
    ...
    public function annualFeessummaryCharts($branchID = '', $studentID = '')
    {
        ...
        for ($month = 1; $month <= 12; $month++) {
            $sql = "SELECT IFNULL(SUM(gd.amount), 0) as total_amount,
            IFNULL(SUM(fp.amount),0) as total_paid,
            IFNULL(SUM(fp.discount),0) as total_discount 
            FROM fee_allocation as fa INNER JOIN fee_groups_details as gd ON gd.fee_groups_id = fa.group_id 
            INNER JOIN fee_payment_history fp ON fp.allocation_id = fa.id and fp.type_id = gd.fee_type_id 
            WHERE MONTH(gd.due_date) = " . $this->db->escape($month) . " AND YEAR(gd.due_date) = YEAR(CURDATE()) 
            AND fa.session_id = " . $this->db->escape(get_session_id());
            if (!empty($branchID))
