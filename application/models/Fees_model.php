...
class Fees_model extends MY_Model
{
    ...
    public function getInvoiceStatus($studentID)
    {
        ...
        fee_allocation.student_id = " . $this->db->escape($studentID) . " AND fee_groups_details.student_id = 0 AND fee_allocation.session_id = " . $this->db->escape(get_session_id());
        ...
        fee_allocation.student_id = " . $this->db->escape($studentID) . "  AND fee_payment_history.student_id = 0 AND fee_allocation.session_id = " . $this->db->escape(get_session_id());
        $paid = $this->db->query($sql)->row_array();
		//print $paid['amount'] + $paid['discount']."<br>";
    	//print $balance['total'];die;
    	//
        if ($paid['amount'] == 0) {
            $status = 'unpaid';
    ...
    {
        $sql = "SELECT fee_allocation.id as allocation_id, fee_allocation.group_id, fees_type.name, fee_groups_details.amount,fee_groups_details.id as fees_g_id, fee_groups_details.student_id, fee_groups_details.due_date, fee_groups_details.fee_type_id FROM fee_allocation LEFT JOIN
    ...
    public function getStuPaymentHistory($classID='', $SectionID='', $paymentVia, $start, $end, $branchID, $onlyFine=false,$typeID='',$staff_id ='')
    {
        ...
        if (!empty($SectionID)){
     	...
     	if (!empty($typeID)) {
            ...
            //print_r($fees);die;
            $this->db->where_in('h.type_id', $fees);
            } else {
    	...
    	if(!empty($staff_id)){
    		 $this->db->where('h.collect_by', $staff_id);
        }
