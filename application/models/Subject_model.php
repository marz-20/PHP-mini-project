...
class Subject_model extends MY_Model
{
    ...
    public function get_subject_list($class_id, $section_id)
    {
    	...
    	$this->db->where('sa.subject_type', 'regular');
	...
	// get subject list by class id and section id
    public function get_subject_extra_list()
    {
        $this->db->select('sa.*,s.name,b.name as branch_name,c.name as class_name,se.name as section_name,st.name as staff_name,stu.first_name,stu.last_name');
        $this->db->from('subject_assign as sa');
        $this->db->join('subject as s', 's.id = sa.subject_id', 'left');
    	$this->db->join('branch as b', 'b.id = sa.branch_id', 'left');
    	$this->db->join('class as c', 'c.id = sa.class_id', 'left');
    	$this->db->join('section as se', 'se.id = sa.section_id', 'left');
    	$this->db->join('staff as st', 'st.id = sa.teacher_id', 'left');
    	$this->db->join('student as stu', 'stu.id = sa.student_id', 'left');
        $this->db->where('sa.subject_type', 'extra');
        $subjects = $this->db->get()->result();
        
        return $subjects;
    }

	public function get_subject_extra_by_id($id)
    {
        $this->db->select('*');
